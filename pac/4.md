...
* ## <font color="crimson">简单函数的拉氏变换</font>
	* ### <font color="royalblue">单位阶跃函数</font>$1(t)$
		$$1(t) = \begin{cases} 0 & \text{$t$<0} \\ 1 & \text{$t \geq 0$ } \end{cases} $$
	...
	* ### <font color="royalblue">单位速度函数</font>
		...
		$$L[f(t)]  =\int_{0}^{\infty}te^{-st}dt  = t\frac{e^{-st}}{-s}\big|_{0}^{\infty}-\int_{0}^{\infty}\frac{e^{-st}}{-s}dt=\frac{1}{s^2}(Re(s)>0)$$
		[单位速度函数](./img/sudu.png)
